...
export const ProModal: React.FC<ProModalProps> = ({ isOpen, onClose, onActivate }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const activated = onActivate(code);
    if (activated) {
      setSuccess(true);
      setError(false);
      setTimeout(() => {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText('1028587072');
    setCopied(true);
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                <input
                  ...
                  className={`w-full px-4 py-3 rounded-xl border-2 focus:outline-none focus:ring-2 focus:ring-offset-1 transition-all text-center font-mono uppercase tracking-widest ${error ? 'border-red-300 focus:ring-red-200 bg-red-50' : 'border-slate-200 focus:border-indigo-500 focus:ring-indigo-200'}`}
                />
                {error && <p className="text-red-500 text-xs mt-1">Mã không đúng. Vui lòng kiểm tra lại.</p>}
              </div>
              <button
                type="submit"
                className="w-full bg-indigo-600 hover:bg-indigo-700 active:scale-95 transition-all text-white font-bold py-3 rounded-xl shadow-lg shadow-indigo-200 flex items-center justify-center"
              >
